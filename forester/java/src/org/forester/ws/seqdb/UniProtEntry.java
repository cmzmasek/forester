        ...
        for (final String line : lines) {
            if (line.startsWith("AC")) {
                ...
                if (line.indexOf("(strain") > 0) {
                    e.setStrain(SequenceDbWsTools.extractFromTo(line, "(strain", ")"));
                }
            } else if (line.startsWith("RC") && (line.indexOf("STRAIN=") > 0) && (line.indexOf("{") > 0) && ForesterUtil.isEmpty(e.getStrain())) {
                e.setStrain(SequenceDbWsTools.extractFromTo(line, "STRAIN=", "{"));
            } else if (line.startsWith("RC") && (line.indexOf("STRAIN=") > 0) && (line.indexOf(";") > 0) && ForesterUtil.isEmpty(e.getStrain())) {
                e.setStrain(SequenceDbWsTools.extractFromTo(line, "STRAIN=", ";"));
            } else if (line.startsWith("OH") && (line.indexOf("NCBI_TaxID=") > 0)) {
                e.setViralHost(SequenceDbWsTools.extractFromTo(line, ";", "."));
