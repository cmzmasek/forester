...
public final class AncestralTaxonomyInference {
    ...
    private static void inferTaxonomyFromDescendentsSimple(final PhylogenyNode n) throws IOException, AncestralTaxonomyInferenceException {
        if (n.isExternal()) {
        ...
        if (n1.getNodeData().isHasTaxonomy() && n2.getNodeData().isHasTaxonomy() && n1.getNodeData().getTaxonomy().getLineage() != null && n2.getNodeData().getTaxonomy().getLineage() != null) {
            ...
            int s = lin1.size() <= lin2.size() ? lin1.size() : lin2.size();
    ...
    private static void inferTaxonomyFromDescendents(final PhylogenyNode n) throws IOException, AncestralTaxonomyInferenceException {
        if (n.isExternal()) {
            throw new IllegalArgumentException("attempt to infer taxonomy from descendants of external node");
        }
        n.getNodeData().setTaxonomy(null);
        ...
        for (final PhylogenyNode desc : descs) {
            if (desc.getNodeData().isHasTaxonomy() && (TaxonomyDataManager.isHasAppropriateId(desc.getNodeData().getTaxonomy()) || !ForesterUtil.isEmpty(desc.getNodeData().getTaxonomy().getScientificName()) || !ForesterUtil.isEmpty(desc.getNodeData().getTaxonomy().getLineage()) || !ForesterUtil.isEmpty(desc.getNodeData().getTaxonomy().getTaxonomyCode()) || !ForesterUtil.isEmpty(desc.getNodeData().getTaxonomy().getCommonName()))) {
                final UniProtTaxonomy up_tax = TaxonomyDataManager.obtainUniProtTaxonomy(desc.getNodeData().getTaxonomy(), null, null);
        ...
        if (shortest_lin_length > 0) {
            I:
            for (int i = 0; i < shortest_lin_length; ++i) {
        ...
        for (final PhylogenyNode desc : descs) {
            if (!desc.isExternal() && desc.getNodeData().isHasTaxonomy() && desc.getNodeData().getTaxonomy().isEqual(tax)) {
