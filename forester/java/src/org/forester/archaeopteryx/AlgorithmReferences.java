...
final class AlgorithmReferences {
    ...
    static List<Reference> all() {
        ...
        refs.add( new Reference( "Alignment conservation / consensus track (Settings → Overlays → Conservation track)",
                "A bar per alignment column under the alignment display, with the column's consensus residue beneath "
                        + "it. The consensus is the most common NON-gap residue (ties resolve to the alphabetically "
                        + "first, so a figure is reproducible). The track offers three choices: two BAR measures, "
                        + "both on [0,1], and a SEQUENCE LOGO that draws the second of them as letters. All three are "
                        + "scored over the tips CURRENTLY DISPLAYED, so entering a subtree or collapsing a clade "
                        + "re-scores the profile for the sequences actually shown.\n"
                        + "CONSENSUS IDENTITY: the fraction of ROWS carrying the consensus residue. Gaps count in the "
                        + "denominator, so a column that is half gaps cannot exceed 0.5.\n"
                        + "INFORMATION CONTENT: the sequence-logo measure, (log2(K) - H) / log2(K), where H is the "
                        + "Shannon entropy of the residues observed in the column and K is the alphabet size (4 for "
                        + "nucleotide, 20 for amino acid), multiplied by the non-gap fraction so gaps cost the same as "
                        + "they do for identity. Unlike identity it separates a column split evenly between two "
                        + "residues from one split four ways, though both have the same majority fraction. Ambiguity "
                        + "codes are counted as ordinary distinct symbols, so a column full of them scores LOW rather "
                        + "than being treated as conserved.\n"
                        + "SEQUENCE LOGO: the same information content drawn as a stack of letters instead of a bar "
                        + "-- one stack per column, each letter's height its share of that column's information, the "
                        + "most frequent residue on top. No small-sample correction is applied: over the handful of "
                        + "tips a subtree can leave, it would exceed the maximum information and blank every column. "
                        + "A conserved but half-gapped column is drawn at FULL conservation and HALF height; an "
                        + "all-gap column draws nothing at all.\n"
                        + "Shannon entropy: Shannon CE (1948): \"A Mathematical Theory of Communication\", Bell System "
                        + "Technical Journal 27(3):379–423 and 27(4):623–656. Information content of an alignment "
                        + "column as displayed here: Schneider TD, Stephens RM (1990): \"Sequence logos: a new way to "
                        + "display consensus sequences\", Nucleic Acids Research 18(20):6097–6100. For a survey of "
                        + "conservation scores and their trade-offs: Valdar WSJ (2002): \"Scoring residue "
                        + "conservation\", Proteins 48(2):227–241.\n"
                        + "NOTE: this is deliberately NOT the physico-chemical property score Jalview labels "
                        + "\"Conservation\" (Livingstone CD, Barton GJ (1993): \"Protein sequence alignments: a strategy "
                        + "for the hierarchical analysis of residue conservation\", CABIOS 9(6):745–756), which is "
                        + "defined for amino acids only; this display serves nucleotide alignments equally." ) );
