...
final class TreeText {
    ...
    private static final Set<String> NEXUS_KEYWORDS = new HashSet<>( Arrays.asList( "#nexus", "begin", "end",
            "endblock", "tree", "translate", "taxlabels", "dimensions", "taxa", "trees", "ntax", "format", "matrix",
            "characters", "data" ) );
    ...
    private static List<Span> newickSpans( final String t, final boolean nexus ) {
        ...
        while( i < n ) {
            ...
            if ( c == '\'' ) { // a quoted label: plain, '' is an escaped quote
                int j = i + 1;
                while( j < n ) {
                    if ( t.charAt( j ) == '\'' ) {
            ...
            if ( ( c == '(' ) || ( c == ')' ) || ( c == ',' ) || ( c == ';' ) || ( c == ':' ) || ( c == '=' ) ) {
                ...
                // a number right after ':' (a branch length) or ')' (a support value)
                if ( ( c == ':' ) || ( c == ')' ) ) {
            ...
            // a word (a label, or in Nexus possibly a keyword)
            int j = i;
            ...
            if ( nexus && line_start ) {
                ...
                if ( NEXUS_KEYWORDS.contains( w ) ) {
                    out.add( new Span( i, j, Kind.KEYWORD ) );
                }
            }
            else if ( nexus && ( j > i ) ) {
                final String w = t.substring( i, j ).toLowerCase( Locale.US );
                if ( w.equals( "tree" ) || w.equals( "end" ) || w.equals( "endblock" ) ) {
                    out.add( new Span( i, j, Kind.KEYWORD ) );
                }
            }
            i = Math.max( j, i + 1 );
